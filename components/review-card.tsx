		...
		>
							...
							{[...Array(5).keys()].map((item, index) => (
								<AiFillStar
									...
									className={`stroke-[30px] ${
										index <= rating ? "fill-golden" : "fill-transparent"
									}`}
								/>
							))}
						</div>
			...
			<div className="flex flex-row items-center justify-start gap-3 text-base">
				<div className="flex flex-row items-center justify-start gap-1">
					<AiFillLike className={`${pathname === "/" ? "fill-grey-goose" : "fill-transparent stroke-[50px] stroke-down-river"}`}/>
					<p>{likeCount}</p>
				</div>

				<div className="flex flex-row items-center justify-start gap-1">
					<AiFillDislike className={`${pathname === "/" ? "fill-grey-goose" : "fill-transparent stroke-[50px] stroke-down-river"}`}/>
					<p>{dislikeCount}</p>
				</div>

				<div className="flex flex-row items-center justify-start gap-1">
					<FaCommentAlt className={`${pathname === "/" ? "fill-grey-goose" : "fill-transparent stroke-[50px] stroke-down-river"}`} />
					<p>{commentCount}</p>
				</div>
